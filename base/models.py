...
class Product(models.Model):
    ...
    brand = models.CharField(
        default='Different', max_length=100, null=True, blank=True, choices=BRAND)
    price = models.FloatField(null=True, blank=True)
    discount_price = models.FloatField(null=True, blank=True)
    image = models.URLField(null=True, blank=True)
    thumbnail = models.ImageField(default='pobrane.png')
    description = models.TextField(
        max_length=200, null=True, default='This is test field')
    created = models.DateTimeField(auto_now_add=True)
    is_approved = models.BooleanField(default=False)
    slug = AutoSlugField(
        populate_from=['title'], max_length=50, null=True, blank=True)
    outlet = models.CharField(
        default='', max_length=100, null=True, blank=True, choices=OUTLET)
...
class OrderItem(models.Model):
    ...
    def get_amount_saved(self):
        try:
            percent_save = ((self.get_total_product_discount_price()
                            * 100) / self.get_total_product_price())
...
class Order(models.Model):
    ...
    status = models.CharField(
        default='Processed', choices=STATUS, max_length=30, null=True, blank=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, blank=True)
...
class Ticket(models.Model):
    ...
    ticket_creator = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, blank=True)
...
class Address(models.Model):
    ...
    address_type = models.CharField(
        max_length=10, blank=True, choices=ADDRESS_TYPES, default='Billing')
...
class Message(models.Model):
    ...
    slug = AutoSlugField(
        populate_from=['subject'], max_length=50, null=True, blank=True)
