...
class AuthorizationTestCase(TestCase):
    ...
    def test_authorized_user_is_able_to_add_to_cart(self):
        ...
        product = Product.objects.create(
            title='test1234',  price=200, slug='test1234')
...
class ProductsaActionTestCase(TestCase):
    ...
    def test_adding_blank_comment(self):
        ...
        self.assertEqual(Comment.objects.last().body,
                         "test123")  # body from setUp comment
    ...
    def test_delete_comment(self):
        ...
        comment = Comment.objects.create(
            user=self.user, product=self.product, body='test123425')
...
class CartTestCase(TestCase):
    ...
    def test_remove_from_cart(self):
        ...
        '''
        product = Product.objects.create(
            title='secondTest', price=200, slug='secondTest')
        url = reverse('add-to-cart', kwargs={'slug': product.slug})
        self.client.get(url)
        url_delete = reverse('remove-from-cart',
    ...
    def test_total_price_in_cart(self):
        ...
        product1 = Product.objects.create(
            title='test1', price=200.4, slug='test12345')
        product2 = Product.objects.create(
            title='test12', price=500.2, slug='test123456')
        product3 = Product.objects.create(
            title='test123', price=400.2, slug='test1234567')
    ...
    def test_add_single_item_to_cart(self):
        ...
        '''
        product1 = Product.objects.create(
            title='test1', price=200, slug='test12345')
        product2 = Product.objects.create(
            title='test12', price=200, slug='test123456')
        self.client.get(reverse('add-to-cart', kwargs={'slug': product1.slug}))
        self.client.get(reverse('add-to-cart', kwargs={'slug': product2.slug}))
        self.client.get(reverse('add-to-cart', kwargs={'slug': product1.slug}))
        self.client.get(reverse('remove-single-item-from-cart',
...
class RegularUserTestCase(TestCase):
    ...
    def test_user_can_read_messages_from_inbox(self):
        ...
        self.client.force_login(user)
        url = reverse('message')
        self.client.post(url, {'subject': 'Test Subject',
                         'body': 'Test body', 'receiver': self.user})
        message = MessageReceiver.objects.filter(receiver=self.user)
        response = self.client.get(
            reverse('message-read', kwargs={'pk': message[0].id}))
        self.assertEqual(response.status_code, 302)
    ...
    def test_user_can_view_message_details(self):
        ...
        message = MessageReceiver.objects.filter(receiver=self.user)
        response = self.client.get(
            reverse('message-detail', kwargs={'pk': message[0].id}))
        self.assertEqual(response.status_code, 200)
...
class TicketTestCase(TestCase):
    ...
    def test_user_can_create_ticket(self):
        url = reverse('ticket')
        self.client.post(
            url, {'subject': 'Test subject1', 'body': 'Test body1'})
